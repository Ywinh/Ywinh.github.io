...
- 程序入口：ELF内部的e_entry，一般对应bootloader里面设置的_start
...
- 程序退出：分为两种，exit syscall和m5_exit
...
![](https://calculet.feishu.cn/space/api/box/stream/download/asynccode/?code=ODU2ZGJlYjMwNWZmNjE3NDY4OTQ2MTkzYjMxNDI5YjVfakZXdmxuTHFrbUpDemthZVBDbk52dXE4N09wWWc5Z1NfVG9rZW46RVZOcGJnaGxOb1VCamR4c3RIc2NJOEdpbk9kXzE3ODI2NDAzMDU6MTc4MjY0MzkwNV9WNA&add_watermark=true&scene_type=CCM)
...
2. l1 dcache load miss：硬件值 79207，gem5 114743
    ...
    4. 推测：硬件对load miss有合并机制，write a case verify it？
        ...
        2. miss合并机制在gem5中表现为mshr，如何逼近到硬件的mshr？ 当同一个 cache block 已经有一个未完成 miss 时，后续对同一 block 的请求会命中已有 MSHR，而不是再新开一个 miss
...
3. dram未对齐
    ...
    2. 但是内部DDR4有合法性检查，不能真的把延时设为0，那么把这个延时设置得很小，ps级别，基本可忽略
        
        - 为什么仅仅修改延时，也会导致访存次数这些的改变
            
    3. 尝试更改ddr4为一个简单memory，但是这会导致访存次数这些也有很大的改变，why？
